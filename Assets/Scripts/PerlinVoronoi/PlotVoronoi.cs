...
public class PlotVoronoi : MonoBehaviour
{
    ...
    void OnValidate()
    {
        ...
        //store the positions and colors of the Voronoi cell centers
        ...
        while(locations.Count < locationCount)
        {
            ...
            if (!locations.ContainsKey(new Vector2Int(x, y)))
            {
                ...
                //texture.SetPixel(x, y, Color.black);// this line is used to visualize the center points for each voronoi cell region
            }
        }
        ...
        for (int y = 0; y < texture.height; y++)
        {
            for (int x = 0; x < texture.width; x++)
            {
                ...
                {
                    float distTo = Vector2Int.Distance(val.Key, new Vector2Int(x, y));//calculate the distance between the center point and each pixel
                    if (distTo < distance)
                    {
                        colour = val.Value;
                        distance = distTo;
